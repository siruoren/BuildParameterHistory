...
- **任务级别配置**：在每个任务的配置页面中单独设置最大记录数，覆盖全局配置
- 支持所有标准 Jenkins 构建参数
- **国际化支持**：根据浏览器语言设置自动切换中文和英文
- **记录选择与批量删除**：选择多条记录并批量删除（需要管理员权限）
- **清除历史**：选项清除任务的所有构建参数历史（需要管理员权限）
...
除了全局配置外，每个任务还可以单独设置最大记录数：

1. 打开任务的配置页面
2. 找到 **Build Parameter History** 配置区域
3. 在 **Max Records**（最大记录数）字段中输入所需的数值（留空则使用全局配置）
4. 点击 **Save**（保存）

**优先级规则：**
- 如果任务配置了最大记录数（非空），则使用任务配置的值
- 如果任务未配置（为空），则使用全局配置的值
